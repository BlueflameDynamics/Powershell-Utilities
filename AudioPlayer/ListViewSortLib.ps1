...
These properties are used by the Sort-ListView Function to track information
...
{
	param([Parameter(Mandatory)][Windows.Forms.ListView]$LvwControl)
	$Value = [PSCustomObject][Ordered]@{LastColumnClicked=-1;LastColumnAscending=$False;Initalized=$True}
...
{
	param(
		[parameter(Mandatory,Position=0)][Windows.Forms.ListView]$LvwControl,
		[parameter(Mandatory,Position=1)][UInt32]$Column)
	...
	$EvalExpression = {if($Numeric){[Double]$_[0]}else{[String]$_[0]}}
